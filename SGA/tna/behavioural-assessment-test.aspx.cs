...
namespace SGA.tna
{
    public partial class behavioural_assessment_test : Page
    {
        ...
        {
            SGACommon.AddPageTitle(this.Page, "Leadership Assessment ", "");
            ...
            SGACommon.IsTakeTest("viewPMPTest");
            if (!base.IsPostBack)
            {
                this.testId = System.Convert.ToInt32(SqlHelper.ExecuteScalar(CommandType.StoredProcedure, "spInitalizeBaTest", new SqlParameter[]
				{
					new SqlParameter("@userId", SGACommon.LoginUserInfo.userId),
					new SqlParameter("@testDate", System.DateTime.UtcNow.ToString()),
					new SqlParameter("@sessionId", this.Session.SessionID)
				}));
                this.dsOptions = SqlHelper.ExecuteDataset(CommandType.StoredProcedure, "spGetBAOptions");
                this.LoadAllTopics();
                this.SetClass();
                this.setPrevButton();
        ...
        private void SetClass()
        {
            for (int i = 0; i < this.rptrTopics.Items.Count; i++)
            {
                ...
                if (i == this.PageNumber)
                {
                    ...
                    this.lblDescription.Text = SqlHelper.ExecuteScalar(CommandType.StoredProcedure, "spGetBATopicDetail", param).ToString();
        ...
        {
            DataSet ds = SqlHelper.ExecuteDataset(CommandType.StoredProcedure, "spGetBaQuestions", new SqlParameter[]
			{
        ...
        {
            this.rptrTopics.DataSource = SqlHelper.ExecuteDataset(CommandType.StoredProcedure, "spGetBATopics");
        ...
        protected void parentRepeater_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.AlternatingItem || e.Item.ItemType == ListItemType.Item)
            {
                ...
                if (lblQuestionText != null)
                {
                    if (hintWord.Length > 0)
                    {
                        ...
                        if (pos != -1)
                        {
                            ...
                            lblQuestionText.Text = lblQuestionText.Text.Replace("( )", "");
				...
				{
					"SELECT isNull(optionId,'') from  tblBaOptions where optionId = (select optionId from tblBaQuestionReply where testId=",
					this.testId,
					" and questionId=",
					questionId,
					")"
        ...
        protected void btnSubmitNext_Click(object sender, System.EventArgs e)
        {
            foreach (RepeaterItem itm in this.parentRepeater.Items)
            {
                ...
                if (rdb != null)
                {
                    ...
                    SqlHelper.ExecuteNonQuery(CommandType.StoredProcedure, "spUpdateBaOptions", param);
                }
            }
            if (this.pgNumber.Value.Length > 0)
            {
        ...
        {
            this.Session["baTestId"] = this.testId;
            string strLink = "";
            foreach (RepeaterItem itm in this.parentRepeater.Items)
            {
                RadioButtonList rdb = (RadioButtonList)itm.FindControl("rbtnlst");
            ...
            if (resultFound.Length > 0)
            {
                ...
                bool isAdded = isdnAPI.addToGroup(Id, 406);
                isdnAPI.dsUpdate("Contact", Id, new XmlRpcStruct
				{
					{
						"_LeadershipTNALink",
						"http://" + base.Request.UrlReferrer.Host + "/LeadershipReport.aspx?Id=" + strLink
					}
				});
            }
            this.testId = 0;
            ...
            base.Response.Redirect("~/tna/my-results-bar-graph-ba.aspx", false);
        }
    }
}
